...
import { Search, AlertTriangle, Clock, ShieldCheck } from 'lucide-react';
...
export function FleetManagementPage({ vehicles, onSelectVehicle }: FleetManagementPageProps) {
  const [activeTab, setActiveTab] = useState<DateType>('lease');
  ...
  const tableTitle = activeTab === 'lease' ? 'סיום ליסינג' : 'סיום רישוי';

  const suppliers = useMemo(() => getUniqueSuppliers(vehicles), [vehicles]);
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [vehicleRows, dateField, filterUrgency, filterSupplier, search]);
