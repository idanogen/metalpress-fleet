-- ============================================================================
-- Migration: fix_maintenance_field_name
-- Date:      2026-08-06
-- Project:   Supabase mbodppnsdnmlejdldztp (schema: fleet)
-- ----------------------------------------------------------------------------
-- מטרה: עמודת maintenance_cost ("טיפולי רכב") תתמלא סוף סוף מפריוריטי.
--
-- הבאג: הפונקציה קראה את השדה בשם המלא EDPE_MAINTENANCEPRICE, אבל פריוריטי
-- מקצר שמות שדות ארוכים, והשם בפועל הוא EDPE_MAINTENANCEPR. השדה שביקשנו
-- פשוט לא קיים ב-payload, אז v_maintenance היה NULL בכל שורה, בכל חודש,
-- מאז היום הראשון. maintenance_cost = 0 בכל ההיסטוריה.
--
-- הוכחה חיה (ריצת הסנכרון של 6/8/2026, 07:02): פריוריטי החזיר עבור רכב
-- 50880203 את הערך "EDPE_MAINTENANCEPR": 1677.97, בזמן שהעמודה בדשבורד 0.
-- במקביל, fleet.vehicle_invoices מכילה 19,708 ש"ח בקטגוריית "טיפולי רכב"
-- ל-2026 שמעולם לא הגיעו לסיכומים המצרפיים.
--
-- התיקון: coalesce על שני השמות. השם המקוצר הוא מה שפריוריטי שולח היום,
-- והשם המלא נשאר כגיבוי אם אי פעם ייחשף. שורה אחת השתנתה, שאר הפונקציה
-- זהה לגרסת 2026-07-03.
--
-- הבהרה למה insurance_cost ו-license_cost נשארים 0: זה תקין ולא באג.
-- EDPE_INSURANCEPRICE ו-EDPE_LICENSEPRICE קיימים ב-payload אך ריקים בפריוריטי,
-- ואין להם קטגוריית חשבוניות כלל. אצל רכבי ליסינג הם מגולמים בדמי השכירות.
--
-- אחרי ההחלה: ריצה ידנית אחת של סנריו 4646251 ממלאת רטרואקטיבית את עלויות
-- הטיפולים בכל ההיסטוריה (הסנכרון שולח את כל החודשים בכל ריצה).
-- ============================================================================
...
BEGIN
  ...
  IF jsonb_typeof(p_payload->'METL_CARUSAGE_SUBFORM') = 'array' THEN
    ...
    LOOP
      ...
      -- פריוריטי מקצר שמות שדות ארוכים: השם בפועל הוא EDPE_MAINTENANCEPR.
      -- השם המלא נשאר כגיבוי בלבד למקרה שייחשף בעתיד.
      v_maintenance := coalesce(
        nullif(v_report->>'EDPE_MAINTENANCEPR', '')::numeric,
        nullif(v_report->>'EDPE_MAINTENANCEPRICE', '')::numeric
      );
      ...
      -- מצב השורה הקיימת לפני ה-upsert: קובע ספירה (insert/update/skip)
      -- וגם את זיהוי הסתירות מול פריוריטי בשורות מוגנות.
      v_existing_mileage := NULL;
      ...
      ON CONFLICT (vehicle_id, report_year, report_month) DO UPDATE SET
        -- ק"מ מוגן ברמת שדה: שורת בוט/ידני שומרת את קריאת הנהג, ובשורת
        -- פריוריטי ערך 0 מפריוריטי לא דורס קריאה אמיתית שכבר קיימת.
        mileage = CASE
          WHEN fleet.monthly_reports.source <> 'priority' THEN fleet.monthly_reports.mileage
          WHEN EXCLUDED.mileage > 0 THEN EXCLUDED.mileage
          ELSE fleet.monthly_reports.mileage END,
        -- עלויות: פריוריטי הוא מקור האמת היחיד להן — מתעדכנות תמיד, גם
        -- בשורות בוט/ידני (הנהג לא מדווח עלויות, אין מה להגן).
      ...
      ELSIF v_existing_source = 'priority' THEN
        v_reports_updated := v_reports_updated + 1;
      ELSE
        -- שורה מוגנת (בוט/ידני): העלויות עודכנו, הק"מ של הנהג נשמר.
        -- סתירות ק"מ מול פריוריטי מדווחות לדף הדיווחים החריגים — כמו קודם:
