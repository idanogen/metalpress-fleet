-- ============================================================================
-- Migration: costs_always_sync_on_protected_rows
-- Date:      2026-07-03
-- Project:   Supabase mbodppnsdnmlejdldztp (schema: fleet)
-- ----------------------------------------------------------------------------
-- מטרה: עמודות העלות (14 קטגוריות) מתעדכנות מפריוריטי תמיד — גם כששורת החודש
-- נוצרה מדיווח וואטסאפ/ידני. ההגנה על דיווח הנהג נשארת, אבל ברמת שדה הק"מ
-- בלבד, לא ברמת השורה כולה.
--
-- הבאג: ה-ON CONFLICT DO UPDATE היה עם WHERE source='priority' — מנעול ברמת
-- שורה. שורה שנוצרה ע"י הבוט (source='whatsapp_bot') דילגה על כל העדכון,
-- כולל 14 עמודות העלות שהנהג בכלל לא מדווח עליהן. התוצאה: כל רכב שהנהג שלו
-- דיווח בוואטסאפ לפני הסנכרון נשאר בלי עלויות לאותו חודש.
-- הוכחה (מאי 2026): 73/110 שורות בוט, דשבורד הציג ~143K הוצאות מול ~412K+
-- בפועל (לפי vehicle_invoices). 46 רכבים עם חשבוניות אך אפס עלויות בשורה.
--
-- התיקון:
--   1. ה-WHERE הוסר — העדכון רץ תמיד בקונפליקט.
--   2. mileage מוגן ב-CASE: שורת בוט/ידני שומרת את קריאת הנהג; בשורת פריוריטי
--      ערך 0 מפריוריטי לא דורס קריאה אמיתית (כמו קודם).
--   3. זיהוי הסתירות (divergence flagging) עבר מ"העדכון דולג" (NOT FOUND)
--      לבדיקה מפורשת של source לפני ה-upsert — אותה התנהגות, אותם תנאים.
--   4. reports_skipped ממשיך לספור שורות מוגנות (הק"מ לא עודכן בהן).
--
-- אחרי ההחלה: ריצה ידנית אחת של סנריו 4646251 ממלאת רטרואקטיבית את העלויות
-- החסרות בכל ההיסטוריה (הסנכרון שולח את כל החודשים בכל ריצה).
-- ============================================================================
