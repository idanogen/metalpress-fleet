...
### החוב הוא של הנהג, לא של הרכב שרשום עליו עכשיו
תזכורת הק"מ של 15/8/2026 יצאה ל-4 נהגים מתוך 29 שכבר דיווחו בזמן (איריס שרביט רז
חודשיים ברצף). `runMonthlySend` שאל "האם לרכב הנוכחי של הנהג יש דיווח לחודש", ולכן:
- **החלפת רכב מוחקת את הדיווח מהחישוב.** הדיווח יושב על הרכב הישן, והסנכרון מפריוריטי
  כבר ניתק אותו (`current_driver_id=null`, `is_active=false`), אז הוא לא נמצא בשאילתה.
  זה לא נדיר: איריס עברה 152 ⟵ 169 ⟵ 171 בשלושה חודשים.
- **רכב שהתקבל אחרי סוף חודש הדיווח נספר כחסר**, למרות שאי אפשר לדווח עליו רטרואקטיבית.
- 🔴 **הדשבורד כבר ידע לסנן את זה** (`isApplicableForMonth` ב-`src/lib/analytics.ts`),
  אבל מנוע השליחה בשרת לא השתמש בו. **לוגיקה שמוצגת למנהל ולוגיקה ששולחת ללקוח חייבות
  להיות אותה לוגיקה** — כאן המסך הראה נכון והוואטסאפ שיקר, ורק הלקוחה גילתה.
- התיקון: `loadPeriodReports` + `needsReminder` ב-`api/_lib/monthly-messaging.ts`.
- **לפני כל שינוי במיקוד השליחה: להריץ `?dry=1`** על `/api/send-monthly-reminder`
  (מחזיר את רשימת היעד בלי לשלוח ובלי לכתוב ליומן) ולהשוות מול הכלל הקודם.
  אין דרך אחרת לבדוק את זה בלי להתיז וואטסאפ ל-100 נהגים.

### 99 שליחות בדקה = חמש נדחות בשקט (מגבלת הקצב של heyy)
ב-1/9/2026 ההודעה החודשית יצאה ל-99 נהגים ב-46 שניות, ו-heyy מגבילה את הדייר ל-**100
קריאות לדקה** בחלון קבוע, משותף לכל הערוצים (גם ר.שעל) ולוובהוק שעונה לנהגים. החמישה
האחרונים ברשימה (בהם איריס שרביט רז) קיבלו 429 ונרשמו `failed` ביומן, ואיש לא ראה עד
שאיריס שאלה. ב-1/8 אותה ריצה לקחה 48 שניות ועברה, כלומר היינו על הקצה מההתחלה.
- **התיקון (8/9/2026):** ריווח `SEND_SPACING_MS` (1 שנייה) בין שליחות, ועל 429 המתנה
  עד `x-ratelimit-reset` וניסיון חוזר (`sendTemplate` ב-`api/_lib/monthly-messaging.ts`).
  `maxDuration: 300` לשתי פונקציות השליחה ב-`vercel.json`, כי הריצה עכשיו ~100 שניות.
- **שליחה שנכשלה לא נשלחת שוב לבד.** ההרצה מחדש של אותו endpoint מדלגת על `accepted`
  ושולחת רק לכושלים, אז "לנסות שוב" = להריץ שוב עם `x-sync-secret`.
- **מייל כשל לאיריס (8/9/2026, בקשת עידן):** כשנשארים כשלים אחרי הניסיון החוזר,
  `api/_lib/failure-mail.ts` בונה מייל בערכת הבית (`api/_lib/emailkit.ts`) עם טבלת
  הנהגים שלא קיבלו, ושולח לאיריס + עידן דרך וובהוק מייק **4909614** ("עוגן — שליחת
  מייל מערכת", hook 2815862, payload `{to, subject, html}`, to מופרד בפסיקים) → ג'ימייל
  4660042. ביום תקין לא יוצא מייל. נמענים ב-`FAILURE_MAIL_TO`, כתובת ב-`MAIL_WEBHOOK_URL`
  (ברירות מחדל בקוד). הסנריו גנרי ומיועד לשימוש חוזר בכל מערכת שצריכה מייל בלי ספק מייל.
